...
CCircularCache::CCircularCache(size_t front, size_t back, bool bDoubleCache)
 ...
 , m_end2(-1)
 , m_time2(0)
 , m_start2(bDoubleCache ? (m_size / 2) : -1)
 , m_cur(0)
 , m_buf(NULL)
 , m_size(front + back)
 , m_size_back(back)
...
int CCircularCache::Open()
{
  ...
  m_beg2 = -1;
  m_end2 = -1;
  m_start2 = (m_double_cache ? (m_size / 2) : -1);
  m_cur  = 0;
  return CACHE_RC_OK;
}
...
int CCircularCache::WriteToCache(const char *buf, size_t len)
{
  ...
  if (m_cur >= m_beg1 && m_cur <= m_end1)
  {
    ...
    pos    = (m_start1 + back1 + front1) % m_size;
